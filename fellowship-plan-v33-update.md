...
description: Design and status for the social/community feature — profiles, posts, DMs, group chats, block/report (v1, shipped v22) — PLUS the v2 social-media redesign (likes/comments/repost/save, follow, notifications, Explore, Stories, Shorts, hamburger nav) shipped v23 — PLUS v24/v25/v27 hotfixes from Jared's live real-account test, a v26 fix for a real firestore.rules bug (a null-resource dereference on the very first DM between two real accounts) that v25's diagnostic error code (permission-denied) helped pin down, a v28 fix for a second, same-shape firestore.rules bug (a missing-field dereference on the very first directory/{uid} write for a new profile), a v29 fix for real accounts being unable to find each other in People search at all plus a PWA update-prompt banner, a v30 fix adding the ability to actually set/change your display name from My Profile, a v31 pass adding DM message notifications (previously a disclosed gap, now built) and replacing v29's update banner with a fully automatic silent update (no click needed) per Jared's follow-up ask -- plus a netlify.toml fix for a secrets-scanning false positive blocking Jared's Netlify deploy, and clarification that Jared's Netlify site needs its FULL repo replaced with each new full-source zip, not just individual files added alongside old code -- a v32 pass that restores the permission-based update banner (reversing v31, per Jared's follow-up ask) and fixes the header's own avatar disappearing on every reload until visiting My Profile -- and a v33 pass fixing the story-viewer DELETE button overflow (now a kebab menu), adding Facebook-style story-ring thumbnail previews, fixing the worship-session Bible search (reference parsing + a threshold bug), and adding a title slide to the front of every song's presentation.
status: v1, v2, and the v24 through v33 hotfixes/features are all shipped. v24-v28 are confirmed working on Jared's real Firebase project as of 2026-09-09. v29-v33 are not yet independently confirmed live -- every confirmation attempt so far has hit either the Netlify secrets-scan failure or a stale-repo-content deploy issue (both now fixed/clarified), so the true end-to-end test (full repo replace + successful deploy + fresh page load) is still pending Jared's next retry. DM/message push notifications are built (v31) -- no longer a disclosed gap. The update-prompt mechanism is, as of v32, back to the permission-based banner (v29's behavior), after a brief detour through a fully silent auto-update in v31. Real (server-delivered) push notifications -- distinct from the in-app notification bell above -- are approved by Jared but not yet built; see `claude/architecture-and-decisions.md` for the research/plan. A story layout editor (text overlays, background colors/gradients, stickers/emoji, drawing tool) is approved and scoped but not yet built.
...
## v32: update banner restored (again), and the header avatar disappearing on every reload [2026-09-10]

Two changes this pass, both direct reversals/fixes of things reported right after v31 shipped.

**Update banner restored, reversing v31's silent auto-update.** Jared tested v31's fully-silent auto-update, then asked for the opposite behavior back: "oh then make sure the prompt comes about updating." So `onNeedRefresh` goes back to calling `showUpdateBanner(updateSW)` (the v29 UPDATE NOW/LATER banner) instead of v31's silent `updateSW(true)`. `vite.config.js` needed no changes either time -- `registerType:'prompt'` already just controls whether the service worker installs-and-waits versus taking over immediately; the prompt-vs-silent decision has lived entirely in `app.js`'s callback the whole time. The banner's CSS (`.update-banner*` in `styles.css`, using the theme-safe `--surface`/`--ink`/`--border`/`--ground` tokens rather than `--wine`/`--pine`, which invert lightness between light/dark mode and would break contrast as a solid banner background) is back too. This is now documented in-line in both files with the full v29-31-32 history, so a future pass doesn't re-litigate the same back-and-forth a third time without knowing it already happened twice.

**Header avatar disappearing on every reload, fixed.** Jared: "the profile picture disappears from the top everytime I reload the app, I'd have to go to profile first then go back so it appears again." Reproduced the exact mechanism by tracing the code: the header's own avatar (next to the "iWorship" logo, and the hamburger-menu avatar) is rendered by `personAvatar(state.user.uid, ...)`, which reads photo/name from `state.directory` (via `directoryEntry(uid)`) -- NOT from `state.profile`. `state.directory` is only populated once `startDirectoryWatch()` runs, and that watch is deliberately only started from specific view-open handlers (Sermons, Messages, My Profile, Admin, Explore, Shorts, Notifications, feed, DM/group-chat threads -- see that function's own comment for why: it's not needed app-wide the way the social watches are). So right after a fresh reload, before any of those screens has been opened this session, `state.directory` is empty and the header avatar falls back to the '?' placeholder -- exactly matching "disappears... until I go to profile first."

**Fix.** `personAvatar()` now falls back to `state.profile` specifically when rendering the SIGNED-IN user's own avatar (`uid === state.user.uid`) and the directory entry isn't loaded yet or doesn't have a photo yet. `state.profile` is populated immediately and reliably on every sign-in via `watchProfile()` (see `watchAuth()`), so the header's own avatar now shows correctly from the very first render after a reload, with no dependency on which screen happens to be opened first. Once the directory entry does load with its own photo, it takes over again (the fallback only kicks in when the directory entry is missing or photo-less) -- so this doesn't change behavior once things sync up, only closes the gap in between. Other people's avatars are completely unaffected: the fallback is gated strictly to the signed-in user's own uid, so nobody else's photo can leak in from `state.profile`.

**Testing.** Syntax-checked (`node --check src/app.js`). Rather than relying only on the existing UI-level Playwright suite (which drives full sign-in/view flows and would need a specific "reload before opening any directory-watch screen" scenario added to catch this precisely), the fixed function's exact logic was extracted and run against seven hand-built scenarios covering: the reported bug case (own avatar, directory not loaded yet, profile has a photo -- now correctly shows the photo, previously would have shown '?'), directory loaded but not yet carrying a photo, directory already synced with its own photo (confirmed it still wins once available), another person's avatar with an empty directory (confirmed unaffected, still shows '?' -- no leakage of the signed-in user's own photo), another person's avatar once their directory entry loads normally, signed-out state (no crash), and the initial-letter fallback when no photo exists anywhere. All seven passed. The full 163-check Playwright suite was also re-run in demo mode with zero regressions -- demo mode's `local-data-layer.js` recomputes its directory list live from profiles rather than gating on a separate watch, so it was never able to reproduce this particular bug in the first place, same disclosed-limitation pattern as v26/v28/v29's directory-self-heal fix; confirmation that the header avatar now survives a real reload depends on Jared's next test.

**Deploy**: `iworship-deploy-v32.zip` / `iworship-full-source-v32.zip`. No `firestore.rules` change -- both fixes are `src/app.js`/`src/styles.css` only. **Jared: same as v31, this needs your ENTIRE repo replaced with this zip's contents, then committed/pushed** -- not individual files added alongside what's already there. After redeploying, reload the app fresh (not just navigate within it) and confirm the header avatar shows immediately without needing to visit My Profile first, and confirm the update banner (rather than a silent reload) is what you see next time a new version ships after this one.

## v33: story-viewer fixes, story previews, Bible search fixes, and a title slide for every song [2026-09-10]

Four separate reports/asks landed back-to-back while push-notification work (see `claude/architecture-and-decisions.md`) was starting, and all four are bundled into this one pass rather than shipped as separate partial deploys, per the standing plan to bundle remaining work given Jared's stated Netlify-credit concern.

**Story-viewer DELETE button was overflowing/colliding with the close button, fixed.** Jared sent screenshots showing the word "DELETE" visually crashing into the adjacent × close button in the story viewer. Root cause: `.icon-btn-sm` is a fixed 30×30px class meant for a single glyph (×, a heart, etc.); using it for the multi-letter word "DELETE" forced it wider than its box, overlapping the neighboring close button. Fixed by replacing the DELETE text button with a kebab menu (⋮) that opens a small dropdown containing "Delete Story" — the same pattern used elsewhere in the app for an options menu that only sometimes applies (own content only). Clicking outside the dropdown closes it; choosing "Delete Story" closes the dropdown and runs the existing delete flow unchanged. Verified fixed with a dedicated Playwright smoke test.

**Facebook-style story-ring previews, added.** Jared: "it's good if there was a preview of the story of your friends like in facebook." Story rings previously always showed a person's plain avatar regardless of whether they had an active story; they now show an actual thumbnail of the story's own photo/video (a small `<img>` or muted `<video>` cropped into the ring) behind the usual colored ring border, falling back to the plain avatar only when a person has no active story. Your own "Your Story" ring does the same — it shows your own latest story's thumbnail behind the "+" add-badge once you have one live.

**Bible search in the worship-session lookup, fixed.** Jared: "fix the search function cause it can't even search for verses properly." Two distinct bugs, found by reading the code rather than guessing: (1) the on-screen search box attempted a search at 2 typed characters, but the actual search function underneath silently requires 3 — so any 2-character query always, incorrectly, reported "0 matches," which is exactly what "can't even search" describes; (2) typing an actual reference like "John 3:16" was never recognized as one — despite the search box's own placeholder implying it — and instead ran a literal (near-always-failing) text search over the verse body. Both fixed: the on-screen threshold now matches the real 3-character minimum with a clearer "keep typing" hint, and a reference is now parsed and shown directly (with a READ-IN-FULL button to jump to that chapter) before falling through to a text search.

Separately, Jared asked to make the worship-session Bible screen "almost the same as EasyWorship where... you can select the verses by hand as well." The session's Bible screen already has exactly that as its default mode when there's no search query — browse by book, then chapter, then tap any verse to stage/display it — so no new screen was built for this; it's flagged here in case Jared was picturing something further (a persistent verse-picker panel alongside the stage view, for instance) so that can be scoped as a separate ask rather than assumed satisfied.

**Title slide added to the front of every song.** Jared: "also add a title slide for all the songs." Every song now presents an extra slide before its first real lyric section, showing the song's own title (and its key, when set, as "KEY OF <key>") in a large centered style distinct from a normal lyric slide. Built as a pure, derived view — a new `songSectionsForPresenting(song)` helper prepends the synthetic title slide onto a song's real `sections` array only at the point of use (staging/going live, the projector/stage view, the musician chart view, host status text and jump chips, congregant split view) — the underlying `song.sections` data itself, the song editor, and the library's own "N sections" count are completely untouched, so the synthetic slide can never leak into an edit, be saved, or corrupt a song's real data. One correctness fix came out of this while implementing it: the musician chart view's "NOW" highlight reads the live section index directly, so it had to be updated to use the same derived list too — otherwise, whenever the title slide was the one actually live, the chart would have highlighted the wrong (off-by-one) section as current. Caught and fixed before it could ever surface as a live bug.

**Testing.** Each fix verified with a dedicated Playwright smoke test (story kebab menu + delete flow + ring thumbnails; the title slide's slide-1-of-N staging, going live, and the projector/stage render). One pre-existing regression test (`test-preview-live.cjs`) needed its own fix, not an app fix: it asserted specific label text ("Section 2") that was only ever true because the old fixed demo layout happened to render that literal digit — with the title slide now occupying index 0, the same ArrowRight-once action correctly lands on a different, still-correct section whose label no longer needs to contain a "2". Confirmed via direct HTML inspection that the new behavior is correct, then rewrote the check to compare before/after text rather than depend on specific wording — 10/10 passing again, zero app-code changes resulting from that particular investigation. The full existing regression suite continues to pass alongside these new checks.

**Deploy**: `iworship-deploy-v33.zip` / `iworship-full-source-v33.zip`. No `firestore.rules` change -- this pass is `src/app.js`/`src/styles.css` only. **Jared: same as v31/v32, this needs your ENTIRE repo replaced with this zip's contents, then committed/pushed** -- not individual files added alongside what's already there. After redeploying: open a story with the new kebab menu and confirm Delete Story still works; check that friends' story rings show their actual photo/video behind the ring; try searching the Bible in a worship session with both a short word and a real reference like "John 3:16"; and pick any song to confirm it now opens on a title slide before the first lyric.
